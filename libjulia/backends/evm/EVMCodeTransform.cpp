...
{
	solAssert(m_scope, "");
	for (auto const& varName: _variableNames | boost::adaptors::reversed)
	{
		auto var = m_scope->lookup(varName.name);
		if (var)
		{
		...
		else
		{
			...
			m_identifierAccess.generateCode(varName, IdentifierContext::LValue, m_assembly);
		}
	}
}
